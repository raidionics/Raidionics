...
class NeuroDiagnosisParameters:
    ...
    def to_txt(self, filename):
        ...
        for t in self.statistics['Main']['Overall'].mni_space_tracts_overlap.keys():
            pfile.write('  * {} atlas\n'.format(t))
            tracts_ordered = collections.OrderedDict(sorted(self.statistics['Main']['Overall'].mni_space_tracts_overlap[t].items(), key=operator.itemgetter(1), reverse=True))
            ...
            pfile.write('\n')
            tracts_ordered = collections.OrderedDict(sorted(self.statistics['Main']['Overall'].mni_space_tracts_distance[t].items(), key=operator.itemgetter(1), reverse=False))
            for r in tracts_ordered.keys():
    ...
    def to_csv(self, filename):
        ...
        column_names.extend([#'ExpectedResectableVolume (ml)',
                             'ExpectedResidualVolume (ml)', 'ResectionIndex',
                             # 'ComplexityIndex'
                             ])
        ...
        for t in self.statistics['Main']['Overall'].mni_space_tracts_overlap.keys():
            for r in self.statistics['Main']['Overall'].mni_space_tracts_overlap[t].keys():
                values.extend([self.statistics['Main']['Overall'].mni_space_tracts_overlap[t][r]])
                column_names.extend([t + r.split('.')[0][:-4] + '_overlap'])

        for t in self.statistics['Main']['Overall'].mni_space_tracts_distance.keys():
            for r in self.statistics['Main']['Overall'].mni_space_tracts_overlap[t].keys():
                values.extend([self.statistics['Main']['Overall'].mni_space_tracts_distance[t][r]])
